...
public class JavascriptWrapFactory extends WrapFactory {
    ...
    public Object wrap(Context cx, Scriptable scope, Object obj, Class<?> staticType) {
        ScriptableObject coercedValue = this.coerce(staticType, obj, scope);
        if (coercedValue != null) return coercedValue;
        return super.wrap(cx, scope, obj, staticType);
    }

    public <T> ScriptableObject coerce(Class<?> type, T value, Scriptable scope) {
        @SuppressWarnings("unchecked")
        Coercer<T, ?> coercer = (Coercer<T, ?>) COERCERS.get(type);
        if (coercer == null) return null;
        return coercer.coerceValue(value, scope);
    }
